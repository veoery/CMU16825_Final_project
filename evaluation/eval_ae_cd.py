...
# Try importing utils and cadlib, provide fallbacks
try:
    from utils import read_ply
except ImportError:
    print("Warning: utils.py not found, using basic PLY reader")
    def read_ply(filepath):
        """Basic PLY reader fallback"""
        try:
            import trimesh
            mesh = trimesh.load(filepath)
            return np.array(mesh.vertices)
        except:
            raise ImportError("Cannot read PLY file. Install trimesh: pip install trimesh")

try:
    from cadlib.visualize import vec2CADsolid, CADsolid2pc
    CADLIB_AVAILABLE = True
except ImportError:
    print("Warning: cadlib not available. Chamfer distance evaluation requires real CAD conversion.")
    print("Install from: https://github.com/DavidXu-JJ/DeepCAD (branch: tags/CAD-MLLM)")
    CADLIB_AVAILABLE = False

    # Mock functions for testing data format only
    def vec2CADsolid(vec):
        """Mock function - cannot create real CAD solids without cadlib"""
        raise NotImplementedError("vec2CADsolid requires cadlib installation")

    def CADsolid2pc(shape, n_points, data_id):
        """Mock function - cannot convert CAD to point cloud without cadlib"""
        raise NotImplementedError("CADsolid2pc requires cadlib installation")

# Configurable paths
PC_ROOT = os.environ.get('PC_ROOT', "../data/pc_cad")
SKIP_DATA = []
