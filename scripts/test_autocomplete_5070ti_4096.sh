#!/bin/bash
# Ultra-conservative test for 5070 Ti - seq_len=4096
# Run this in WSL with conda environment activated: conda activate p3d_5070ti

echo "============================="
echo "Testing Autocompletion on 5070 Ti (seq_len=4096)"
echo "============================="

# Navigate to project root
cd "$(dirname "$0")/.." || exit

# Ultra-conservative configuration for 5070 Ti (16GB VRAM)
BATCH_SIZE=1
GRAD_ACCUM=16
MAX_SEQ_LEN=4096  # Very conservative
LORA_R=8  # Minimal LoRA rank
MAX_SAMPLES=100  # Test with just 100 samples

# Data paths (use WSL paths, not Windows paths)
DATA_ROOT="./data/Omni-CAD-subset-complete"
